...
namespace VXA
{
	...
	{
		private List<Label> stackLabels = new List<Label>();
		public List<Label> StackLabels
		{
		...
		public List<Label> DataLabels
		{
			get { return dataLabels; }
			set { dataLabels = value; }
		}

		private List<Label> codeLabels = new List<Label>();
		public List<Label> CodeLabels
		{
		...
		public void FindLabels(StreamReader source)
		{
			...
			while (line != null)
			{
				...
				else if (line.Contains(":"))
				{
					...
					if (segment == "code")
					{
						codeLabels.Add(new Label(label, codeAddress));
					...
					else if (segment == "data")
					{
						...
						dataLabels.Add(new Label(label, dataAddress));
					...
					else if (segment == "stack")
					{
						...
						stackLabels.Add(new Label(label, stackAddress));
		...
		public void ReadInstructions(StreamReader source)
		{
			string line = source.ReadLine();
			string segment = "";
			int codeAddress = 0;
		...
		private int TranslateLabel(string name)
		{
			...
			label = stackLabels.Find(delegate(Label l) { return l.Name == name; });
			if (label != null)
			{
				throw new Exception("Stack labels can't be referenced");
			}

			label = dataLabels.Find(delegate(Label l) { return l.Name == name; });
			if (label != null)
			{
				return label.Address;
			}

			label = codeLabels.Find(delegate(Label l) { return l.Name == name; });
			if (label != null)
			{
				return label.Address;
			}
