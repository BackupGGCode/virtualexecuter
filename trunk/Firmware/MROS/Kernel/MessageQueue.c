...
unsigned char index;
	...
	if(mq->full)
	{
		...
		Kernel_DebuggerEvent(DEBUG_EVENT_POSTMESSAGE, null, DEBUG_STATUS_NO_ROOM, (unsigned long)mq);
#endif
		return false;
	}
...
#if TASKER_DEBUG_LEVEL > 0
			Kernel_DebuggerEvent(DEBUG_EVENT_POSTMESSAGE, null, DEBUG_STATUS_OK, (unsigned long)mq);
#endif
			return true;
		}
	}
...
#if TASKER_DEBUG_LEVEL > 0
		Kernel_DebuggerEvent(DEBUG_EVENT_GETMESSAGE, null, DEBUG_STATUS_NO_MESSAGE, (unsigned long)mq);
#endif
		return false;
	}
...
#if TASKER_DEBUG_LEVEL > 0
	Kernel_DebuggerEvent(DEBUG_EVENT_GETMESSAGE, null, DEBUG_STATUS_OK, (unsigned long)mq);
#endif
	return true;
}
