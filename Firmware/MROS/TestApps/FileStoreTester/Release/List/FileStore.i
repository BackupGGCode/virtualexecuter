...
extern bool FileStore_OpenFile(unsigned char* name, fsfile* file);
extern bool FileStore_OpenFile(unsigned char* name, fsfile* file);
extern bool FileStore_GetFileName(fsfile* file, unsigned char* name);
extern unsigned long FileStore_ReadBytes(fsfile* file, unsigned char* data, unsigned short length);
extern unsigned long FileStore_ReadLine(fsfile* file, unsigned char* data, unsigned short maxLength);
extern bool FileStore_GetFileEntry(fsfile* file, bool getFirst);
...
bool MatchFileName(unsigned char* name, unsigned long address);
...
bool FileStore_OpenFile(unsigned char* name, fsfile* file)
{
unsigned long index=0, newIndex;

	while(MatchFileName(name, index)==(0))
	{
	...
	file->index=index;
	file->start=readLong(index+1);
	file->size=readLong(index+5);
	file->current=0;
...
unsigned long next = current;
	...
	if((readByte(current) & 0x03) == (1<<0))
	{
		next += 10 + readByte(current + 9);
	}
...
bool MatchFileName(unsigned char* name, unsigned long index)
{
	if((readByte(index) & 0x03) != (1<<0))
	{
		return (0);
	}
