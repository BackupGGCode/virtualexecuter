...
void _delay_us(unsigned short t)
{
unsigned char temp;
...
static unsigned char Enc28j60Bank=0xff;
static unsigned char NextPacketPtr;


#define CS_LOW																	ENC28J60_CS_PORT&=~(1<<ENC28J60_CS_BIT)
#define CS_HIGH																	ENC28J60_CS_PORT|=(1<<ENC28J60_CS_BIT)

#define RESET_LOW																ENC28J60_RESET_PORT&=~(1<<ENC28J60_RESET_BIT)
#define RESET_HIGH															ENC28J60_RESET_PORT|=(1<<ENC28J60_RESET_BIT)
...
void NIC_Init(unsigned char* macaddr)
{
	// initialize I/O
	
	// ss as output:
	ENC28J60_CS_DDR |= (1<<ENC28J60_CS_BIT);
	ENC28J60_INTERRUPT_DDR &= ~(1<<ENC28J60_INTERRUPT_BIT);
	ENC28J60_RESET_DDR |= (1<<ENC28J60_RESET_BIT);
	CS_HIGH;
...
void NIC_SendBlocking(unsigned char* buffer, unsigned short length)
{
	...
	enc28j60WriteBuffer(length, buffer);
...
unsigned short NIC_Get(unsigned char* buffer, unsigned short maxLength)
{
	...
	if((rxstat & 0x80)==0)
		...
		enc28j60ReadBuffer(len, buffer);		// copy the packet from the receive buffer
